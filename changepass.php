...
if (isset($_POST['changepassword'])) {
    ...
    $querya = "SELECT * from admintable where adminname='fenny' ";
    $chg_pwd = mysqli_query($conn, $querya);
    $chg_pwd1 = mysqli_fetch_array($chg_pwd);
    $data_pwd = $chg_pwd1['password'];
    if ($data_pwd == $currentpassword) {
        if ($newpassword == $confirmpassword) {
            $queryb = "UPDATE admintable set password='$newpassword' where adminname='fenny'";
            $update_pwd = mysqli_query($conn, $queryb);
            echo "<script>alert('Update successfully'); window.location='adminloginde.php'</script>";
        } else {
        ...
        <div class="col-xs-4 col-xs-push-4" style="background-color: #ffffff;">
            <div class="panel panel-default">
                <div class="panel-body">
                    <form class="form-horizontal  content" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post"
                          role="form">
                        <div class="form-group">
                            <label>Current Password :</label>
                            <input type="password" class="form-control " name="currentpassword" id="password"
                                   autocomplete="off" placeholder="current password"></input>
                            <br>
                            <label>New Password :</label>
                            <input type="password" class="form-control " name="newpassword" id="password"
                                   autocomplete="off" placeholder="new password"></input>
                            <br>
                            <label>Confirm Password :</label>
                            <input type="password" class="form-control " name="confirmpassword" id="password"
                                   autocomplete="off" placeholder="confirm password"></input>
                        </div>
                        <div class="form-group">
                            <center>
                                <button type="submit" name="changepassword" class="btn passcg btn-default">change
                                    password
                                </button>
                                &nbsp&nbsp
